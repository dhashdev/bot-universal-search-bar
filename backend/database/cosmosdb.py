...
class CosmosDB:
    ...
    # def create_item(self, question: str, name: str, result: Dict[str, Any]) -> str:
    ...
    # def delete_item(self, user_id, conversation_id):
    #     query = f"SELECT * FROM c WHERE c.user_id = '{user_id}' AND c.conversation_id = '{conversation_id}'"
    #     items = list(self.container.query_items(query=query, enable_cross_partition_query=True))

    #     if items:
    #         item_to_delete = items[0]
    #         self.container.delete_item(item=item_to_delete, partition_key=user_id)
    #         return f"Delete {conversation_id} with user_id {user_id} successfully"
    #     else:
    #         return f"No item found for user_id {user_id} and conversation_id {conversation_id}"
